#include "map/DynamicCulling.h"
#include <fstream>
#include <iostream>

namespace RVWO {
DynamicCulling::DynamicCulling()
    : mbAcceptKeyFrames(true), mbFinishRequested(false), mbFinished(true)
...
void DynamicCulling::Run() {
  ...
  cout << "Median K-Means time: "
       << vTimesDetectKmeans[vTimesDetectKmeans.size() / 2] << endl;
  cout << "Mean K-Means time: " << totaltime / vTimesDetectKmeans.size()
       << endl;
  tStatics.open("time_statistic_K-Means.txt");
  tStatics << "Total KFs K-Means processed: " << vTimesDetectKmeans.size()
           << endl;
  tStatics << "Median K-Means time: "
           << vTimesDetectKmeans[vTimesDetectKmeans.size() / 2] << endl;
  tStatics << "Mean K-Means time: " << totaltime / vTimesDetectKmeans.size()
           << endl;
  tStatics.close();
  SetFinish();
