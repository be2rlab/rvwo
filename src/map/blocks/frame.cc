...
Frame::Frame(const Frame &frame)
    : mpcpi(frame.mpcpi), mpORBvocabulary(frame.mpORBvocabulary),
      ...
      mpCamera(frame.mpCamera), mpCamera2(frame.mpCamera2), Nleft(frame.Nleft),
      Nright(frame.Nright), monoLeft(frame.monoLeft),
...
Frame::Frame(const cv::Mat &imLeft, const cv::Mat &imRight,
    ...
    : mpcpi(NULL), mpORBvocabulary(voc), mpORBextractorLeft(extractorLeft),
      ...
      mThDepth(thDepth), mpPrevFrame(pPrevF),
      mpReferenceKF(static_cast<KeyFrame *>(NULL)), mbIsSet(false),
      mpCamera(pCamera), mpCamera2(nullptr), mbHasPose(false),
      mbHasVelocity(false) {
  // Frame ID
  mnId = nNextId++;
...
Frame::Frame(const cv::Mat &imLeft, const cv::Mat &imRight,
  ...
  cv::Vec3b PEOPLE_COLOR(61, 5, 150); // BGR !!!
  // int PEOPLE_COLOR = 1172;
  for (size_t i(0); i < mvKeys.size(); ++i) {
    cv::Vec3b val = imSeg.at<cv::Vec3b>(mvKeys[i].pt.y, mvKeys[i].pt.x);
    // std::cout << (int)val.val[0] << " " << (int)val.val[1] << " " <<
    // (int)val.val[2] << " " << std::endl; int val =
    // (int)imSeg.at<uchar>(mvKeys[i].pt.y,mvKeys[i].pt.x);

    if (val == PEOPLE_COLOR) {
      mvSegVal.push_back(1.0f);
      mvDynKeys.push_back(mvKeys[i]);
      mDynDescriptors.push_back(mDescriptors.row(i));
    } else {
      mvSegVal.push_back(0.0f);
    }
    // double prob = ; <- The function for probability vector filling is being
    // called over here mvProb.push_back() <- Over here the vector is being
    // filled
  }

  N = mvKeys.size();
  int N_dyn = mvDynKeys.size();
  // cout << "Dyn keys: " << N_dyn << endl;
  if (mvKeys.empty())
    return;
  ...
  //        tbb::parallel_for(
  //                tbb::blocked_range<size_t>(0, mvKeys.size()),
  //                [&](tbb::blocked_range<size_t> rmvKeys) {
  //                    for (size_t j = rmvKeys.begin(), jend = rmvKeys.end(); j
  //                    < jend; j++) {
  //                        float radii = 20.0f;
  //                        int level = mvKeys[j].octave;
  //                        vector<size_t> features_in_area =
  //                        GetFeaturesInArea(mvKeys[j].pt.x,
  //                                                                            mvKeys[j].pt.y,
  //                                                                            radii,
  //                                                                            level,
  //                                                                            level);
  //                        for (auto &it: features_in_area) {
  //                            if (mvSegVal[it] == 1.0f && mvSegVal[j] != 1.0f)
  //                            {
  //                                mvSegVal[j] = 0.9f;
  //                                mvDynKeys.push_back(mvKeys[j]);
  //                                mDynDescriptors.push_back(mDescriptors.row(j));
  //                            }
  //                        }
  //                    }
  //                });
  ...
  for (size_t j(0); j < mvKeys.size(); ++j) {
    float radii = 20.0f;
    // TODO: Propagate the prior probability according to the distance from the
    // kp
    int level = mvKeys[j].octave;
    vector<size_t> features_in_area =
        GetFeaturesInArea(mvKeys[j].pt.x, mvKeys[j].pt.y, radii, level, level);
    for (auto &it : features_in_area) {
      if (mvSegVal[it] == 1.0f && mvSegVal[j] != 1.0f) {
        // TODO: Here can be exponential propagation (consider the distance and
        // the radii also)
        mvSegVal[j] = 0.9f;
        mvDynKeys.push_back(mvKeys[j]);
        mDynDescriptors.push_back(mDescriptors.row(j));
...
Frame::Frame(const cv::Mat &imGray, const cv::Mat &imDepth,
    ...
    : mpcpi(NULL), mpORBvocabulary(voc), mpORBextractorLeft(extractor),
      ...
      mpReferenceKF(static_cast<KeyFrame *>(NULL)), mbIsSet(false),
      mpCamera(pCamera), mpCamera2(nullptr), mbHasPose(false),
      mbHasVelocity(false), depth_(imDepth.clone()) {
  // Frame ID
  mnId = nNextId++;
  // imDepth.convertTo ( depth_, CV_32F, cfg_->cam_depth_factor_ );
...
// Constructor for RGB-D with Seg cameras.
Frame::Frame(const cv::Mat &imGray, const cv::Mat &imDepth,
             const cv::Mat &imSeg, const double &timeStamp,
    ...
    : mpcpi(NULL), mpORBvocabulary(voc), mpORBextractorLeft(extractor),
      mpORBextractorRight(static_cast<ORBextractor *>(NULL)),
      depth_(imDepth.clone()), gray_(imGray.clone()), mTimeStamp(timeStamp),
      mK(K.clone()), mK_(Converter::toMatrix3f(K)), mDistCoef(distCoef.clone()),
      mbf(bf), mThDepth(thDepth), mpPrevFrame(pPrevF),
      mpReferenceKF(static_cast<KeyFrame *>(NULL)), mbIsSet(false),
      mpCamera(pCamera), mpCamera2(nullptr), mbHasPose(false),
      mbHasVelocity(false) {
  // Frame ID
  ...
  // int PEOPLE_COLOR = 1172;
  for (size_t i(0); i < mvKeys.size(); ++i) {
    // Somehow it doesn't extract points with given color code
    cv::Vec3b val = imSeg.at<cv::Vec3b>(mvKeys[i].pt.y, mvKeys[i].pt.x);
    // std::cout << (int)val.val[0] << " " << (int)val.val[1] << " " <<
    // (int)val.val[2] << " " << std::endl; int val =
    // (int)imSeg.at<uchar>(mvKeys[i].pt.y,mvKeys[i].pt.x);
    if (val == PEOPLE_COLOR) {
      mvSegVal.push_back(1.0f);
      mvDynKeys.push_back(mvKeys[i]);
      mDynDescriptors.push_back(
          mDescriptors.row(i)); // If not worked we can try to inverse condition
                                // to consider != case (orig)
    } else {
      mvSegVal.push_back(0.0f);
    }
    // double prob = ; <- The function for probability vector filling is being
...
bool Frame::ProjectPointUnDistort(MapPoint *pMP, cv::Point2f &kp, float &u,
                                  float &v) {
  // 3D in absolute coordinates
  Eigen::Vector3f P = pMP->GetWorldPos();
...
Frame::Frame(const cv::Mat &imLeft, const cv::Mat &imRight,
             ...
             Sophus::SE3f &Tlr, Frame *pPrevF)
    : mpcpi(nullptr), mbHasPose(false), mbHasVelocity(false),
      mpORBvocabulary(voc), mpORBextractorLeft(extractorLeft),
      mpORBextractorRight(extractorRight), mTimeStamp(timeStamp), mK(K.clone()),
      mK_(Converter::toMatrix3f(K)), mDistCoef(distCoef.clone()), mbf(bf),
      mThDepth(thDepth), mpOdomPreintegrated(nullptr), mpPrevFrame(pPrevF),
      mpOdomPreintegratedFrame(nullptr),
      mpReferenceKF(static_cast<KeyFrame *>(nullptr)), mpCamera(pCamera),
      mpCamera2(pCamera2) {
...
Frame::Frame(const cv::Mat &imLeft, const cv::Mat &imRight,
    ...
    : mpcpi(NULL), mpORBvocabulary(voc), mpORBextractorLeft(extractorLeft),
      ...
      mK_(Converter::toMatrix3f(K)), mDistCoef(distCoef.clone()), mbf(bf),
      mThDepth(thDepth), mpPrevFrame(pPrevF),
      mpReferenceKF(static_cast<KeyFrame *>(NULL)), mpCamera(pCamera),
  ...
  //                tbb::blocked_range<size_t>(0, mvKeys.size()),
  //                        [&](tbb::blocked_range<size_t> rmvKeys) {
  //                            for (size_t j = rmvKeys.begin(), jend =
  //                            rmvKeys.end(); j < jend; j++) {
  //                                float radii = 20.0f;
  //                                int level = mvKeys[j].octave;
  //                                vector<size_t> features_in_area =
  //                                GetFeaturesInArea(mvKeys[j].pt.x,
  ...
  //                                                                                    level);
  //                                for (auto &it: features_in_area) {
  //                                    if (mvSegVal[it] == 1.0f && mvSegVal[j]
  //                                    != 1.0f) {
  //                                        mvSegVal[j] = 0.9f;
  //                                        mvDynKeys.push_back(mvKeys[j]);
  //                                        mDynDescriptors.push_back(mDescriptors.row(j));
