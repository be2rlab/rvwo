...
MapPoint::MapPoint()
    : mnFirstKFid(0), mnFirstFrame(0), nObs(0), mnTrackReferenceForFrame(0),
      ...
      mbFirstTime(true), mProbMov(0), mReprErr(0), mRelInv(0),
...
MapPoint::MapPoint(const Eigen::Vector3f &Pos, KeyFrame *pRefKF, Map *pMap)
    : mnFirstKFid(pRefKF->mnId), mnFirstFrame(pRefKF->mnFrameId), nObs(0),
      ...
      mpReplaced(static_cast<MapPoint *>(nullptr)), mfMinDistance(0),
      mfMaxDistance(0), mpMap(pMap), mbFirstTime(true), mProbMov(0),
      mReprErr(0), mRelInv(0), mnOriginMapId(pMap->GetId()) {
  SetWorldPos(Pos);
...
MapPoint::MapPoint(const Eigen::Vector3f &Pos, Map *pMap, Frame *pFrame,
    ...
    : mnFirstKFid(-1), mnFirstFrame(pFrame->mnId), nObs(0),
      ...
      mpRefKF(static_cast<KeyFrame *>(nullptr)), mnVisible(1), mnFound(1),
      mbBad(false), mpReplaced(nullptr), mpMap(pMap), mbFirstTime(true),
      mProbMov(0), mReprErr(0), mRelInv(0), mnOriginMapId(pMap->GetId()) {
  SetWorldPos(Pos);
...
void MapPoint::SetWorldPosPrev(const Eigen::Vector3f &Pos) {
  unique_lock<mutex> lock2(mGlobalMutex);
  unique_lock<mutex> lock(mMutexPos);
  mWorldPosPrev = Pos;
}
...
Eigen::Vector3f MapPoint::GetWorldPosPrev() {
  unique_lock<mutex> lock(mMutexPos);
  return mWorldPosPrev;
}
...
Eigen::Vector3f MapPoint::GetNormalPrev() {
  unique_lock<mutex> lock(mMutexPos);
  return mNormalVectorPrev;
}
...
void MapPoint::SetNormalVectorPrev(const Eigen::Vector3f &normal) {
  unique_lock<mutex> lock3(mMutexPos);
  mNormalVectorPrev = normal;
}
