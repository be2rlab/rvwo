...
DynamicDetector::DynamicDetector() {
  ...
  dpc_n_sel_pts_per_cluster_ = 300; // Number of points per cluster to be
                                    // selected for dynamic cluster decision.
  dpc_search_square_size_ = 10;     // 9 pixels

  // ret_tk_db_= 0.20;                 // Base threshold for erroneous match
  // discard. (m) ret_tk_kd_= 0.025;                // Scale factor of the
  // threshold for erroneous match discard.

  ret_tk_db_ = 0.02; // Base threshold for erroneous match discard. (m)
  ret_tk_kd_ =
      0.0; // Scale factor of the threshold for erroneous match discard.
...
void DynamicDetector::removeDynamicPixels(KeyFrame *mKF) {
  ...
  computePointCloud(mKF->depth_, pts2d, pts3d,
                    Eigen::Vector4d(mKF->fx, mKF->fy, mKF->cx, mKF->cy));
  std::chrono::high_resolution_clock::time_point t2 =
      std::chrono::high_resolution_clock::now();
  double tComputePointCloud =
  ...
  int K_clusters =
      std::ceil((double)pts2d.size() / (double)dpc_npts_per_cluster_);
  ...
  double tSegKmeans =
      std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(t4 -
          ...
          .count();
  //// draw
  // drawClustersOnImage ( mKF->image_source_clusters_, width_, height_,
  // clusters_2d, colors_ );
  ...
  dynamicDepthImgCullingByMultiViewConstraint(mKF->depth_, clusters_2d,
                                              dynamic_flags);
  ...
  std::chrono::high_resolution_clock::time_point t6 =
      std::chrono::high_resolution_clock::now();
  double tDetect =
      std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(t6 -
                                                                            t5)
          .count();
  ...
  mKF->selectStaticFeatures();
  // // draw.
  drawDynamicClusters(mKF->image_static_clusters_, clusters_2d, dynamic_flags,
                      colors_);
  ...
  // drawResult ( mKF->depth_, dynamic_objects, clusters_2d, dynamic_flags,
  // mKF->image_mask_ ); cv::imshow("Statics
  // Clusters",mKF->image_static_clusters_); cv::imshow("Dynamics KeysUn",
  // dynImg); cv::waitKey(1);
}
 ...
 *****************************************************************/
void DynamicDetector::computePointCloud(const cv::Mat &depth,
  ...
  for (int v = 0; v < depth.rows; v++) {
    for (int u = 0; u < depth.cols; u++) {
      ...
      if ((dp > cam_dmin_) && (dp < cam_dmax_)) {
        Eigen::Vector3d ptc =
            Converter::img2Cam(Eigen::Vector2d(u, v), dp,
                               K); // This is new smt like unproject I see
        pt3d.push_back(ptc);
...
void DynamicDetector::segmentPointCloudByKmeans(
    const vector<Eigen::Vector2d> &pts2d, const vector<Eigen::Vector3d> &pts3d,
    const int n_clusters, vector<vector<Eigen::Vector2d>> &clusters_2d,
  ...
  cv::kmeans(points, n_clusters, labels, criteria, 3, cv::KMEANS_PP_CENTERS,
             centers); //
...
void DynamicDetector::drawClustersOnImage(
    cv::Mat &io_img, const int width, const int height,
 ...
 *****************************************************************/
void DynamicDetector::detectDynamicClusters(
    Map *pMap, KeyFrame *ckf,
    const vector<vector<Eigen::Vector2d>> &clusters_2d,
    const vector<vector<Eigen::Vector3d>> &clusters_3d,
    vector<bool> &dynamic_flags) {
...
bool DynamicDetector::isDynamicCluster(
    const vector<Eigen::Vector2d> &cluster_2d,
    const vector<Eigen::Vector3d> &cluster_3d, KeyFrame *ckf,
    std::vector<KeyFrame *> near_kfs) {
  const Eigen::Vector4d K_ =
      Eigen::Vector4d(ckf->fx, ckf->fy, ckf->cx, ckf->cy);
  ...
  for (size_t np = 0; np < test_pts_2d.size(); np++) {
    ...
    for (size_t nk = 0; nk < near_kfs.size(); nk++) {
      ...
      Eigen::Vector3d ptc =
          Converter::toSE3Quat((cTwr * Trc_).inverse()).map(pt3d);
      // Point must be in front of the KF.
      if (ptc[2] < 0.1) {
        continue;
      }
      // const Eigen::Vector4d nK = Eigen::Vector4d(ckf->fy,
      // ckf->fy,ckf->cx,ckf->cy);
      ...
      double z_primer = 0.0;
      for (int u = nu[0] - half_search_square_size;
           u <= nu[0] + half_search_square_size; u++) {
        for (int v = nu[1] - half_search_square_size;
             v <= nu[1] + half_search_square_size; v++) {
          ...
          if ((dp > cam_dmin_) && (dp < cam_dmax_)) {
            const Eigen::Vector3d nptc =
                Converter::img2Cam(Eigen::Vector2d(u, v), dp, K_);
            // const Eigen::Vector3d nptw = nTwr * Trc_ * nptc;
            const Eigen::Vector3d nptw =
                Converter::toSE3Quat(nTwr * Trc_).map(nptc);
            // compute distance.
...
void DynamicDetector::drawDynamicClusters(
    cv::Mat &io_img, const vector<vector<Eigen::Vector2d>> &clusters_2d,
    const vector<bool> &dynamic_flags, std::vector<uint8_t> colors) {
...
void DynamicDetector::dynamicDepthImgCullingByMultiViewConstraint(
    cv::Mat &depth, const vector<vector<Eigen::Vector2d>> &clusters_2d,
    const vector<bool> &dynamic_flags) {
  int dyn_count = 0;
  for (size_t i = 0; i < dynamic_flags.size(); i++) {
