...
System::System(const string& strVocFile, const string& strSettingsFile,
  ...
  cv::FileNode nodeKernel = fsSettings["System.Kernel"];
  mKernel = (nodeKernel.empty()) ? 0 : (int)nodeKernel;
  ...
  mpLocalMapper = new LocalMapping(
      ...
      mKernel, strSequence);
  mptLocalMapping = new thread(&RVWO::LocalMapping::Run, mpLocalMapper);
  ...
  mpLoopCloser = new LoopClosing(mpAtlas, mpKeyFrameDatabase, mpVocabulary,
                                 mKernel, mSensor != MONOCULAR,
                                 activeLC);  // mSensor!=MONOCULAR);
  mptLoopClosing = new thread(&RVWO::LoopClosing::Run, mpLoopCloser);
...
System::System(const string &strVocFile, const string &strSettingsFile, const string &pascal_prototxt,
                const string &pascal_caffemodel, const string &pascal_png, const eSensor sensor,
    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    mpLocalMapper = new LocalMapping(this, mpAtlas, mSensor==MONOCULAR || mSensor==IMU_MONOCULAR,
                                     mSensor==IMU_MONOCULAR || mSensor==IMU_STEREO || mSensor==IMU_RGBD, mKernel, strSequence);
    mptLocalMapping = new thread(&RVWO::LocalMapping::Run,mpLocalMapper);
    mpLocalMapper->mInitFr = initFr;
    if(settings_)
        mpLocalMapper->mThFarPoints = settings_->thFarPoints();
    ...
    //Initialize the Loop Closing thread and launch
    // mSensor!=MONOCULAR && mSensor!=IMU_MONOCULAR
    mpLoopCloser = new LoopClosing(mpAtlas, mpKeyFrameDatabase, mpVocabulary, mKernel, mSensor!=MONOCULAR, activeLC); // mSensor!=MONOCULAR);
    mptLoopClosing = new thread(&RVWO::LoopClosing::Run, mpLoopCloser);
    ...
    // mptSegment = new thread(&RVWO::Segment::Run, mpSegment);

    // Initialize the Dynmacic Culling thread and launch
    // mpDynamiCulling = new DynamicCulling();
    // mptDynamicCulling = new thread(&RVWO::DynamicCulling::Run, mpDynamiCulling);
...
Sophus::SE3f System::TrackStereoSegment(const cv::Mat& imLeft, const cv::Mat& imLeftSeg,
                                 ...
                                 string filename) {
  if (mSensor != STEREO && mSensor != IMU_STEREO) {
    cerr << "ERROR: you called TrackStereoSegment but input sensor was not set to "
            "Stereo nor Stereo-Inertial."
         << endl;
    exit(-1);
  }

  cv::Mat imLeftToFeed, imLeftSegToFeed, imRightToFeed;
  if (settings_ && settings_->needToRectify()) {
    cv::Mat M1l = settings_->M1l();
    cv::Mat M2l = settings_->M2l();
    cv::Mat M1r = settings_->M1r();
    ...
    cv::remap(imLeft, imLeftToFeed, M1l, M2l, cv::INTER_LINEAR);
    cv::remap(imLeftSeg, imLeftSegToFeed, M1l, M2l, cv::INTER_LINEAR);
    cv::remap(imRight, imRightToFeed, M1r, M2r, cv::INTER_LINEAR);
  } else if (settings_ && settings_->needToResize()) {
    cv::resize(imLeft, imLeftToFeed, settings_->newImSize());
    cv::resize(imLeftSeg, imLeftSegToFeed, settings_->newImSize());
    cv::resize(imRight, imRightToFeed, settings_->newImSize());
  } else {
    imLeftToFeed = imLeft.clone();
    imLeftSegToFeed = imLeftSeg.clone();
    imRightToFeed = imRight.clone();
  }
  ...
  // mpTracker->GetImg(imLeftSeg);
  Sophus::SE3f Tcw = mpTracker->GrabImageStereoSegment(imLeftToFeed, imLeftSegToFeed, imRightToFeed,
                                                timestamp, std::move(filename));
...
Sophus::SE3f System::TrackRGBDSemantic(const cv::Mat& im, const cv::Mat& depthmap,
  ...
  cv::Mat imToFeed = im.clone();
  cv::Mat imDepthToFeed = depthmap.clone();
  cv::Mat imSegToFeed = imSeg.clone();
  if (settings_ && settings_->needToResize()) {
    cv::Mat resizedIm;
    cv::resize(im, resizedIm, settings_->newImSize());
    imToFeed = resizedIm;
    ...
    cv::resize(depthmap, imDepthToFeed, settings_->newImSize());
    cv::resize(imSeg, imSegToFeed, settings_->newImSize());
  }
  ...
  // mpTracker->GetImg(im);
  Sophus::SE3f Tcw =
      mpTracker->GrabImageRGBDSemantic(imToFeed, imDepthToFeed, imSegToFeed, timestamp, filename);
...
void System::SaveTrajectoryTUM(const string& filename) {
  ...
  list<RVWO::KeyFrame*>::iterator lRit = mpTracker->mlpReferences.begin();
...
and a flag
    ...
    list<RVWO::KeyFrame*>::iterator lRit =
