...
// Constructor for Stereo cameras with semantic
    Frame::Frame(const cv::Mat &imLeft, const cv::Mat &imRight, const cv::Mat &imSeg, const double &timeStamp,
                 ...
                 cv::Mat &distCoef, const float &bf, const float &thDepth, GeometricCamera *pCamera, Frame *pPrevF,
                 const IMU::Calib &ImuCalib)
        ...
        mTimeORB_Ext = std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(
                time_EndExtORB - time_StartExtORB).count();
#endif
        ...
        for (size_t i(0); i < mvKeys.size(); ++i) {
            ...
            // std::cout << (int)val.val[0] << " " << (int)val.val[1] << " " << (int)val.val[2] << " " << std::endl;
            // int val = (int)imSeg.at<uchar>(mvKeys[i].pt.y,mvKeys[i].pt.x);
            ...
            // double prob = ; <- The function for probability vector filling is being called over here
            // mvProb.push_back() <- Over here the vector is being filled
        }
        ...
        mTimeStereoMatch = std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(
                time_EndStereoMatches - time_StartStereoMatches).count();
#endif
...
//                    for (size_t j = rmvKeys.begin(), jend = rmvKeys.end(); j < jend; j++) {
//                        float radii = 20.0f;
//                        int level = mvKeys[j].octave;
//                        vector<size_t> features_in_area = GetFeaturesInArea(mvKeys[j].pt.x,
//                                                                            mvKeys[j].pt.y,
//                                                                            radii,
//                                                                            level,
//                                                                            level);
...
//                });
        ...
        for (size_t j(0); j < mvKeys.size(); ++j) {
            float radii = 20.0f;
            // TODO: Propagate the prior probability according to the distance from the kp
            int level = mvKeys[j].octave;
            vector<size_t> features_in_area = GetFeaturesInArea(mvKeys[j].pt.x,
                                                                mvKeys[j].pt.y,
                                                                radii,
            ...
            for (auto &it: features_in_area) {
                if (mvSegVal[it] == 1.0f && mvSegVal[j] != 1.0f) {
                    // TODO: Here can be exponential propagation (consider the distance and the radii also)
                    mvSegVal[j] = 0.9f;
                    mvDynKeys.push_back(mvKeys[j]);
                    mDynDescriptors.push_back(mDescriptors.row(j));
                }
...
// Constructor for RGB-D with Seg cameras.
    Frame::Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const cv::Mat &imSeg, const double &timeStamp,
            ...
            : mpcpi(NULL), mpORBvocabulary(voc), mpORBextractorLeft(extractor),
              mpORBextractorRight(static_cast<ORBextractor *>(NULL)), depth_(imDepth.clone()), gray_(imGray.clone()),
              mTimeStamp(timeStamp), mK(K.clone()), mK_(Converter::toMatrix3f(K)), mDistCoef(distCoef.clone()), mbf(bf),
              mThDepth(thDepth),
              mImuCalib(ImuCalib), mpImuPreintegrated(NULL), mpPrevFrame(pPrevF), mpImuPreintegratedFrame(NULL),
              mpReferenceKF(static_cast<KeyFrame *>(NULL)), mbIsSet(false), mbImuPreintegrated(false),
        ...
        for (size_t i(0); i < mvKeys.size(); ++i) {
            ...
            if (val == PEOPLE_COLOR) {
                mvSegVal.push_back(1.0f);
                mvDynKeys.push_back(mvKeys[i]);
                mDynDescriptors.push_back(mDescriptors.row(
                        i)); // If not worked we can try to inverse condition to consider != case (orig)
            }
            else {
                mvSegVal.push_back(0.0f);
            }
...
//                });
    ...
    bool Frame::ProjectPointUnDistort(MapPoint *pMP, cv::Point2f &kp, float &u, float &v) {
        // 3D in absolute coordinates
        Eigen::Vector3f P = pMP->GetWorldPos();
