...
Sophus::SE3f Tracking::GrabImageStereoSegment(const cv::Mat& imRectLeft,
                                       const cv::Mat& imLeftSeg,
                                       const cv::Mat& imRectRight,
                                       const double& timestamp,
                                       string filename) {
  mImGray = imRectLeft;
  ...
  if (mSensor == System::STEREO && !mpCamera2)
    mCurrentFrame = Frame(mImGray, imGrayRight, imLeftSeg, timestamp, mpORBextractorLeft,
                          mpORBextractorRight, mpORBVocabulary, mK, mDistCoef,
                          mbf, mThDepth, mpCamera);
  else if (mSensor == System::STEREO && mpCamera2)
    mCurrentFrame = Frame(mImGray, imGrayRight, imLeftSeg, timestamp, mpORBextractorLeft,
    ...
    mCurrentFrame =
        Frame(mImGray, imGrayRight, imLeftSeg, timestamp, mpORBextractorLeft,
              mpORBextractorRight, mpORBVocabulary, mK, mDistCoef, mbf,
              mThDepth, mpCamera, mpCamera2, mTlr, &mLastFrame, *mpImuCalib);
...
Sophus::SE3f Tracking::GrabImageRGBDSemantic(const cv::Mat& imRGB, const cv::Mat& imD, const cv::Mat& imSeg,
                                     const double& timestamp, string filename) {
  mImGray = imRGB;
  cv::Mat imDepth = imD;
  ...
  if (mSensor == System::RGBD)
    mCurrentFrame =
        Frame(mImGray, imDepth, imSeg, timestamp, mpORBextractorLeft, mpORBVocabulary,
              mK, mDistCoef, mbf, mThDepth, mpCamera);
  else if (mSensor == System::IMU_RGBD)
    mCurrentFrame =
        Frame(mImGray, imDepth, imSeg, timestamp, mpORBextractorLeft, mpORBVocabulary,
...
bool Tracking::NeedNewKeyFrame() {
  ...
  if (((c1a || c1b || c1c) && c2) || c3 || c4) {
    ...
    // if( bDynamicsCullingIdle || mpLocalMapper->IsInitializing()) {
      return true;
    } else {
      // mpLocalMapper->InterruptBA();
      if (mSensor != System::MONOCULAR && mSensor != System::IMU_MONOCULAR) {
        if (mpLocalMapper->KeyframesInQueue() < 3)
        // if (mpDynamicCuller->KeyframesInQueue() < 3)
          return true;
        else
          return false;
      } else {
