...
//  const int num_seq = ((argc - 3) / 2);
  ...
  // process frames.
  ORB_SLAM3::System SLAM(argv[1], argv[2], argv[3], argv[4], argv[5], ORB_SLAM3::System::IMU_STEREO, false);
  ...
  for (seq = 0; seq < num_seq; seq++) {
    ...
    for (int ni = first_image[seq]; ni < vnImages[seq]; ni++, proccIm++) {
      ...
      SLAM.TrackStereoSegment(imLeft, imLeftSeg, imRight, tframe, vImuMeas, vOdomMeas);
...
bool LoadImages(const string& strPathFolder, vector<string>& vstrImageLeft,
  ...
  string strPathTimesLeftSeg = strPathFolder + "/fisheye1_seg.txt";
