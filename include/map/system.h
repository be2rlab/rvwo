...
#include "utils/drawer/map_drawer.h"
// #include "Segment.h"
#include "map/tracking.h"
#include "utils/drawer/viewer.h"
...
public:
   ...
   * @param strVocFile: path to vocabulary file
   * @param strSettingsFile: path to settings file
   * @param sensor: sensor type
   * @param bUseViewer: flag to indicate if the viewer is used
   *  @param initFr: initial frame
   * @param strSequence: sequence name
   */
   ...
   * @param strVocFile: path to vocabulary file
   * @param strSettingsFile: path to settings file
   * @param pascal_prototxt: path to the prototxt file
   * @param pascal_caffemodel: path to the caffemodel file
   * @param pascal_png: path to the png file
   * @param sensor: sensor type
   * @param bUseViewer: flag to indicate if the viewer is used
   *  @param initFr: initial frame
   * @param strSequence: sequence name
   */
  System(const string &strVocFile, const string &strSettingsFile,
         const string &pascal_prototxt, const string &pascal_caffemodel,
         const string &pascal_png, const eSensor sensor,
   ...
   * @param vOdomMeas: vector of odometry measurements
  ...
  TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight,
              ...
              const vector<ODOM::Meas> &vOdomMeas = vector<RVWO::ODOM::Meas>(),
              string filename = "");
   ...
   * converted to grayscale. Returns the camera pose (empty if tracking fails).
   * @param imLeft: left image
   * @param imLeftSeg: left image segmentation
   * @param imRight: right image
   * @param timestamp: timestamp
   * @param vOdomMeas: vector of odometry measurements
   * @param filename: filename
   * @return camera pose
   */
  Sophus::SE3f TrackStereoSegment(
      const cv::Mat &imLeft, const cv::Mat &imLeftSeg, const cv::Mat &imRight,
      const double &timestamp,
      const vector<ODOM::Meas> &vOdomMeas = vector<RVWO::ODOM::Meas>(),
      string filename = "");

  /** @brief Process the given rgbd frame. Depthmap must be registered to the
   * RGB frame. Input image: RGB (CV_8UC3) or grayscale (CV_8U). RGB is
   * converted to grayscale. Input depthmap: Float (CV_32F). Returns the camera
   * pose (empty if tracking fails).
   * @param im: image
   ...
   * @param im: image
   * @param depthmap: depthmap
   * @param imSeg: image segmentation
   * @param timestamp: timestamp
   * @param vOdomMeas: vector of odometry measurements
   * @param filename: filename
   * @return camera pose
  ...
  Sophus::SE3f TrackRGBDSemantic(
      const cv::Mat &im, const cv::Mat &depthmap, const cv::Mat &imSeg,
      const double &timestamp,
      const vector<ODOM::Meas> &vOdomMeas = vector<RVWO::ODOM::Meas>(),
      string filename = "");
  ...
  /** @brief This stops local mapping thread (map building) and performs only
   * camera tracking.*/
  void ActivateLocalizationMode();
  /** @brief This resumes local mapping thread and performs SLAM again. */
  void DeactivateLocalizationMode();

  /** @brief Returns true if there have been a big map change (loop closure,
   * global BA) since last call to this function. */
  bool MapChanged();

  /** @brief Reset the system (clear Atlas or the active map). */
  void Reset();
  /** @brief Reset the active map. */
  void ResetActiveMap();
