...
class System {
 ...
 public:
  ...
  System(const string &strVocFile, const string &strSettingsFile,
         const string &pascal_prototxt, const string &pascal_caffemodel, const string &pascal_png,
         const eSensor sensor, const bool bUseViewer = true,
         const int initFr = 0, const string &strSequence = std::string());
  ...
  Sophus::SE3f TrackStereoSegment(
      const cv::Mat& imLeft, const cv::Mat& imLeftSeg, const cv::Mat& imRight, const double& timestamp,
      const vector<IMU::Point>& vImuMeas = vector<ORB_SLAM3::IMU::Point>(),
      const vector<ODOM::Meas>& vOdomMeas = vector<ORB_SLAM3::ODOM::Meas>(),
      string filename = "");
  ...
  Sophus::SE3f TrackRGBDSemantic(
      const cv::Mat& im, const cv::Mat& depthmap, const cv::Mat& imSeg, const double& timestamp,
      const vector<IMU::Point>& vImuMeas = vector<IMU::Point>(),
      string filename = "");    
